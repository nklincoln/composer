...
class NodeDataService extends DataService {
    ...
    */
    ...
    //     LOG.verbose(method, '@PERF ['+ this.stub.getTxID().substring(0, 8) + '] stub.getQueryResult (ms) duration: ' + (Date.now() - t0).toFixed(2));
    //     const t1 = Date.now();
    //     let results = await NodeUtils.getAllResults(iterator, txId);
    //     LOG.verbose(method, '@PERF ['+ this.stub.getTxID().substring(0, 8) + '] NodeUtils.getAllResults (ms) duration: ' + (Date.now() - t1).toFixed(2));
     ...
     * when complete, or rejected with an error.
     */
    async executeQuery(query, batchMode = false) {
        //TEMP CODE, Also how to allow users the option in composer ?
        batchMode = true;
        const method = 'executeQuery';
        LOG.entry(method, query);
        const t0 = Date.now();
        ...
        let iterator = await this.stub.getQueryResult(query);
        let results = await NodeUtils.getAllResults(iterator, batchMode);
        LOG.exit(method, results);
